...
- (id) initWithHex:(NSString *)hex {
    if (self = [self initEmpty]) {
        ...
        //      return nil;
        //    }
        if (!EC_POINT_hex2point(_group, [hex cStringUsingEncoding:NSUTF8StringEncoding], _point, _bnctx)) {
            //    if (!EC_POINT_bn2point(_group, bn, _point, _bnctx)) {
            //      if (bn) BN_clear_free(bn);
            return nil;
        }
...
- (id) initWithX:(BTCBigNumber*)x yBit:(NSInteger)yBit {
    if (self = [self initEmpty]) {
        ...
        if (!EC_POINT_set_compressed_coordinates_GF2m(_group, _point, x.BIGNUM, (int)yBit, _bnctx)) {
            return nil;
        }
...
- (instancetype) multiplyTwo:(BTCBigNumber*)j x:(BTCCurvePoint*)x k:(BTCBigNumber*)k {
    ...
    NSInteger z = 0;
    while (i >= 0) {
        BOOL jBit = BN_is_bit_set(j.BIGNUM, (int)i);
        BOOL kBit = BN_is_bit_set(k.BIGNUM, (int)i);
