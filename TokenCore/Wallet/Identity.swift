...
extension Identity {
    public static func createIdentity(password: String, metadata: WalletMeta) throws -> (String, Identity) {
    ...
    public static func recoverIdentity(metadata: WalletMeta, mnemonic: String, password: String) throws -> Identity {
        let identity = try Identity(metadata: metadata, mnemonic: mnemonic, password: password)
        currentIdentity = identity
        return identity
    }
...
extension Identity {
    ...
    func removeWallet(_ wallet: BasicWallet) -> Bool {
        if let index = keystore.walletIds.firstIndex(where: { return $0 == wallet.walletID }) {
